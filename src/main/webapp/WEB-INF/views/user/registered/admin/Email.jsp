...
<h1>Modificar E-mails</h1>

<c:if test="${(param.success != null)}">

	<c:choose>
		<c:when test="${param.success}">
			<c:set var="successMsg" scope="page"
				value="El perfil fue modificado exitosamente" />
		</c:when>
		<c:otherwise>
			<c:set var="successMsg" scope="page"
				value="Error al modificar el perfil" />
		</c:otherwise>
	</c:choose>
	...
	<span class="block">* <c:out value="${successMsg}" /></span></div>
</c:if>
...
<c:if test="${(errorManager != null)}">
	...
	</span> <c:forEach items="${errorManager.errors}" var="error">
...
<script
	src="<c:out value="${baseURI}" escapeXml="false" />/js/admin/email/init.js"
